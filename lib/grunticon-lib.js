...
(function(){
	...
	Grunticon.prototype._generateLoader = function(){
		...
		var banner = fs.readFileSync( config.files.banner ).toString( "utf-8" );

		min = min.concat( banner )
			.concat( "\n" )
			.concat( uglify.minify( config.files.loader ).code );
		if( config.enhanceSVG ){
			min = min.concat( uglify.minify( config.files.embed ).code );
			if( config.corsEmbed ){
				min = min.concat( uglify.minify( config.files.corsEmbed ).code );
			}
		}
	...
	Grunticon.prototype.process = function(cb){
		...
		// minify the source of the grunticon loader and write that to the output
		var loader = config.min = this._generateLoader();
