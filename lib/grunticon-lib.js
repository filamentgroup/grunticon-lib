...
//	var loadCSSpath = require.resolve( "fg-loadcss" );
	...
	Grunticon.generateLoader = function( enhanceSVG, corsEmbed, logger ){
...
//		min = min.concat( loadcss )
			...
			.concat( "\n" )
			.concat( uglify.minify( files.loader ).code );
		if( enhanceSVG ){
			min = min.concat( uglify.minify( files.embed ).code );
			if( corsEmbed ){
				min = min.concat( uglify.minify( files.corsEmbed ).code );
			}
		}
	...
	Grunticon.prototype.process = function(cb){
		...
		// minify the source of the grunticon loader and write that to the output
		var loader = config.min = Grunticon.generateLoader( config.enhanceSVG, config.corsEmbed, logger );
